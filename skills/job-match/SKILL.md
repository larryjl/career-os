...
Check `outputs/postings/` for an existing file for this Company + Role (glob case-insensitively). If not found, save the posting:

- **Pasted text or text extracted via Chrome (`get_page_text`):** Serialize to a `.docx` using `python-docx` — plain paragraphs, no special formatting. Save to `outputs/postings/[company]_[role]_[YYYY-MM-DD].docx`.
- **Non-LinkedIn URL:** Use `url_to_pdf()` from `skills/_lib/pdf_converter.py` to save as `.pdf` to `outputs/postings/[company]_[role]_[YYYY-MM-DD].pdf`.
- **LinkedIn URL:** Fetch page text via Chrome `get_page_text`, then save as `.docx` (same as pasted text path above).

Never write a `.txt` file — all output must be `.docx` or `.pdf`.
Use `make_filename(company, role, ext)` from `skills/_lib/filename_builder.py` for the filename.
...
2. Add to the `Notes` field in `.claude/state/applications.md` for this row: `Apply via: [instruction summary]` (semicolon-separated from existing notes)
...
Use lowercase-hyphens for `[Company]` and `[Role]` (e.g. `match_report_telus_data-strategist_2026-05-06.md`). Resolve the project root via the `get_project_root()` helper. Save before showing follow-up commands. **Also update `.claude/state/applications.md`** — update the Status and Notes fields for the matching row (matched by Company + Role). If no row exists yet, append one with status `pending`.
...
```
outputs/reports/match_report_[Company]_[Role]_[YYYY-MM-DD].docx
```
...
Also append (or update) the row in `.claude/state/applications.md` per the rules in CLAUDE.md `## Application Tracking`.
