...
Reads the ### ATS-API section of profile/target-companies.md, calls the
...
API_TIMEOUT = 20
API_MAX_RETRIES = 2        # one retry is usually enough for ATS APIs
...
def parse_targets(project_root: str) -> list[dict]:
    """
    Parse the ### ATS-API section of profile/target-companies.md.
    ...
    """
    targets_path = os.path.join(project_root, "profile", "target-companies.md")
    ...
    if not start_m:
        raise ValueError("Could not find '### ATS-API' section in profile/target-companies.md")
