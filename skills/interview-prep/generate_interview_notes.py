...
def _setup_lib_path():
    lib_path = os.path.normpath(os.path.join(SCRIPT_DIR, '../_lib'))
...
def main():
    ...
    output_dir = args.output_dir or os.path.join(project_root, "outputs", "interview-notes")
    os.makedirs(output_dir, exist_ok=True)

    today     = date.today().strftime("%Y-%m-%d")
    filename  = f"prep_{args.company}_{args.role}_{today}"
